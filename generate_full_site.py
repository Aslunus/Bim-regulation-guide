...
for child in list(body):
    if child.tag == W + "p":
        ...
        if style in ("1", "2"):
            ...
            if style == "1":
                doc_nav.append(f'<a href="#{sid}">{esc(text)}</a>')
        else:
        ...
        for tr in child.findall(W + "tr"):
            ...
            if "".join(cells).strip():
                ...
                rows_html.append("<tr>" + "".join(f"<{cell_tag}>{cell}</{cell_tag}>" for cell in cells) + "</tr>")
        if rows_html:
            doc_tables += 1
            doc_blocks += 1
...
with zipfile.ZipFile(xlsx) as archive:
  ...
  <h1>{esc(name)}</h1>
  <p class="intro">Полная таблица листа из файла {esc(xlsx.name)}.</p>
  <div class="meta"><a href="../index.html#mssk">Назад к регламенту</a><span>{len(rows_out)} строк</span><span>{max_col} колонок</span></div>
</header>
<section>
...
<div class="body-grid">
  <nav aria-label="Навигация">
    ...
    <div class="nav-group"><a class="nav-main" href="#docx">Требования DOCX</a><div class="nav-sub">{''.join(doc_nav)}</div></div>
